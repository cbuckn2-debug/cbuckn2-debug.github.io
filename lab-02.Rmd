...
bellevue_stats <- bellevue_homes %>%
  summarise(
    mean_total = mean(total_val, na.rm = TRUE),
    var_total  = var(total_val, na.rm = TRUE)
  )

bellevue_two_lakes <- bellevue_homes %>%
  filter(view_lakewash == 1, view_lakesamm == 1)

bellevue_two_lakes_stats <- bellevue_two_lakes %>%
  summarise(
    mean_total = mean(total_val, na.rm = TRUE),
    var_total  = var(total_val, na.rm = TRUE)
  )
...
diff_stats <- data.frame(
  mean_diff = bellevue_two_lakes_stats$mean_total - bellevue_stats$mean_total,
  var_diff  = bellevue_two_lakes_stats$var_total - bellevue_stats$var_total
)
...
seattle_summary <- kingco_homes_clean %>%
  ...
  group_by(condition) %>%
  summarise(
    avg_val_per_sqft = mean(total_val_per_sqft, na.rm = TRUE),
    property_count   = n()
  ) %>%
...
abline(v = sale_price_mean, col = "green", lwd = 2)
...
So far, we've learned a lots functions, please use as many as possible to explore topics you are interested in about the King County datasets (or any other datasets you are interested in). Please use at least 5 functions below and brief discuss your results.
...
hist(
  sample_means,
  breaks = 50,
  main = "Sampling Distribution of Mean Sale Price (n = 495)",
  xlab = "Mean Sale Price"
)
abline(v = sale_price_mean, col = "yellow", lwd = 2)

set.seed(1)
sample_means <- replicate(
  495,
  bellevue_after2020 %>%
    sample_n(34, replace = TRUE) %>% 
    summarise(avg_price = mean(sale_price)) %>%
    pull(avg_price)
)

kingco_subset <- kingco_homes %>%
  select(city, view_lakewash, view_lakesamm)
kingco_subset <- kingco_homes_clean %>%
  select(city, view_lakewash, view_lakesamm)
kc_recent <- kingco_subset %>%
